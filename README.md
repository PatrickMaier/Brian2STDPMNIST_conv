...
* 28x28x2 refers to a layer of 28x28 horizontally and vertically oriented filters.
* 14x14x2 refers to a layer of 14x14 horizontally and vertically oriented filters.
...
Both commands will write the learned weights to file learned/NETWORK/XeAe-6000.npy and the learned firing thresholds to learned/NETWORK/theta_A-6000.npy. Every 200 images, the commands will also dump intermediate learned weights and firing thresholds into the same subdirectory learned/NETWORK.

In addition, the commmands will also write a heat map visualisation of the learned weights to weights-fig/NETWORK/XeAe-6000.png. Every 20 images, the commands will also dump intermediate visualisations of weights to subdirectory weights-fig/NETWORK.
...
Like before, this command will write the learned weights to file learned/NETWORK/XeAe-6000.npy and the learned firing thresholds to learned/NETWORK/theta_A-6000.npy. Intermediate learned weights and firing thresholds will be written as before, as will visualisations of weights.


## Supervised training (labeling)

The second training phase is a supervised labeling. The trained model is run on the same set of training images, capturing spike count histograms indexed by the training images' labels (classes 0 to 9), for each output neuron. These histograms can be interpreted as a probability distribution for an output neuron's prediction.
...
In order to do prediction, the trained model is run again, on test images, producing spike count histograms. These histograms are interpreted using the spike coount histograms from the respective labeling run. For instance, to predict the first 1000 MNIST test images and evaluate the accuracy of the predictions, run the following commands (assuming weights and firing thresholds have been copied as outlined above).
...
The last command will print overall accuracy and a confusion matrix, amongst other data.
...
The last command will print overall accuracy and a confusion matrix. Since all 1000 images are non-digits, any classification shown by the confusion matrix is a misclassification. That is, only images that aren't classified because no neuron spiked, are correctly recognised as non-digits.


## Prediciton after resetting neurons

The models allow resetting of neurons prior to labeling and testing. In [1], we reset the 10 least confident neurons - which 10 neurons these are depends on the network. Here we demonstrate how to reset neurons 0 to 9 during the labeling and testing phases.
...
It is important that the list of reset neurons is always the same, and that the list (without spaces) is passed as argument to the MNIST_run_model.py and MNIST_eval_runs.py scripts.

The last command will print overall accuracy and a confusion matrix that includes an 11th class, the error class. In this particular case, no image should be classified in the error class, as all input images are digits. However, if the cifar.pickle file is used for testing, all images should be classified as error because none of those images resembles a digit.

Example 2 below shows a full list of commands for testing after resetting neurons.


## Example 1: Training 14x14x2 network for 2 epoch on 3rd batch of 6000 MNIST training images and testing on 5th batch of 1000 MNIST testing images.
...
## Example 2: Resetting 10 neurons of the network trained in example 1 and testing on 1000 CIFAR-100 non-digit images.

We assume that the weights of the model from unsupervised training phases have been copied as outlined above. We repeat the supervised labeling phase after resetting neurons 8, 14, 21, 22, 44, 46, 47, 56, 62, and 72. Then we test, resetting the same list of neurons.
